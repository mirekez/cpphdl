...
{
    // Slices of packed values could lose high bits through an implicit conversion.
    // SystemVerilog slicing must retain exactly WIDTH bits from bits() or pack().
    // Read the packed representation and mask only when the result is below 64 bits.
    if constexpr (detail::has_bits_method<T>::value) {
        uint64_t raw = static_cast<uint64_t>(value.bits(last, first));
        if constexpr (WIDTH == 0) {
            return logic<WIDTH>(0);
        }
        else if constexpr (WIDTH >= 64) {
            return logic<WIDTH>(raw);
        }
        else {
            return logic<WIDTH>(raw & ((1ull << WIDTH) - 1ull));
        }
    }
    else if constexpr (detail::has_pack_method<T>::value) {
        uint64_t raw = static_cast<uint64_t>(value.pack().bits(last, first));
        if constexpr (WIDTH == 0) {
            return logic<WIDTH>(0);
        }
        else if constexpr (WIDTH >= 64) {
...
template<typename T, typename std::enable_if_t<detail::has_pack_method<std::remove_cvref_t<T>>::value && !is_logic_v<std::remove_cvref_t<T>>, int> = 0>
...
template<typename T, typename std::enable_if_t<detail::has_pack_method<T>::value && !is_logic_v<T>, int> = 0>
constexpr uint64_t operator>>(const T& lhs, unsigned rhs)
{
    return static_cast<uint64_t>(lhs.pack()) >> rhs;
}
