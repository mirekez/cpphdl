...
bool Field::print(std::ofstream& out, bool isStruct)
{
    ...
    if (expr.value == "cpphdl_memory") {
        ASSERT1(expr.sub.size() >= 3, std::string("cpphdl_memory subs size = ") + std::to_string(expr.sub.size()) );
    ...
    if (expr.value == "cpphdl_reg") {
        ASSERT1(expr.sub.size() >= 1, std::string("cpphdl_reg subs size = ") + std::to_string(expr.sub.size()) );
        expr.sub[0].flags = Expr::FLAG_REG;
        expr.sub[0].indent = indent + 1;
    ...
    if (expr.type == Expr::EXPR_ARRAY && (expr.sub[1].type == Expr::EXPR_TYPE || expr.sub[1].type == Expr::EXPR_TEMPLATE)) {
        ...
        if (expr.sub[1].value == "cpphdl_reg") {
            ASSERT1(expr.sub[1].sub.size() >= 1, std::string("cpphdl_reg subs size = ") + std::to_string(expr.sub[1].sub.size()) );
