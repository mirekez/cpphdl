...
if ! search_q 'extern "C" void cpphdl_optimized_bind_ports_abi\(void\* obj\)' \
...
if ! search_q 'obj\.output = _ASSIGN_REG\(' \
...
if ! search_q 'p[0-9]+_pointer = std::addressof\(' \
    "$build_dir"/AliasRoot_optimized_combs*.cpp; then
...
if search_q '(^|[^.[:alnum:]_])output_comb_func\(\)' \
    "$build_dir"/AliasRoot_optimized_combs*.cpp; then
...
if search_q 'bool evaluating|s\.evaluating' \
...
if search_q 'if \(s\.evaluat(ed|ing)[0-9]+.*return;' \
...
if search_q 'auto& n[0-9]+ = n0\.forwarding' \
    "$build_dir"/AliasRoot_optimized_combs_dynamic_*.cpp; then
