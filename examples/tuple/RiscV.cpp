...
public:
    ...
    _PORT(uint32_t)  imem_read_addr_out = _ASSIGN_REG( pc );
...
//        auto& wb = std::get<2>(members);
    ...
    void _assign()
    {
        ...
        df.pc_in          = _ASSIGN_REG( pc );
        df.instr_valid_in = _ASSIGN_REG( valid );
        df.instr_in       = imem_read_data_in;
        df.regs_data0_in  = _ASSIGN( df.rs1_out() == 0 ? 0 : regs.read_data0_out() );
        df.regs_data1_in  = _ASSIGN( df.rs2_out() == 0 ? 0 : regs.read_data1_out() );
...
public:
    ...
    void _assign()
    {
        ...
        imem.read_in = _ASSIGN( !imem_write );
        imem.read_addr_in = riscv.imem_read_addr_out;
        imem.write_in = _ASSIGN_REG( imem_write );
        imem.write_addr_in = _ASSIGN_REG( imem_write_addr );
        imem.write_data_in = _ASSIGN_REG( imem_write_data );
        imem.write_mask_in = _ASSIGN( (uint8_t)0xF );
        imem.debugen_in = debugen_in;
        imem.__inst_name = __inst_name + "/imem";
        imem._assign();
#else  // connecting Verilator to CppHDL
        dmem.read_in = _ASSIGN( (bool)riscv.dmem_read_out );
        dmem.read_addr_in = _ASSIGN( (uint32_t)riscv.dmem_read_addr_out );
        dmem.write_in = _ASSIGN( (bool)riscv.dmem_write_out );
        dmem.write_addr_in = _ASSIGN( (uint32_t)riscv.dmem_write_addr_out );
        dmem.write_data_in = _ASSIGN_REG( riscv.dmem_write_data_out );
        dmem.write_mask_in = _ASSIGN( (uint8_t)riscv.dmem_write_mask_out );
        dmem.debugen_in = debugen_in;
        dmem.__inst_name = __inst_name + "/dmem";
        dmem._assign();

        imem.read_in = _ASSIGN( !imem_write );
        imem.read_addr_in = _ASSIGN( (uint32_t)riscv.imem_read_addr_out );
        imem.write_in = _ASSIGN_REG( imem_write );
        imem.write_addr_in = _ASSIGN( (uint32_t)imem_write_addr );
        imem.write_data_in = _ASSIGN( (uint32_t)imem_write_data );
        imem.write_mask_in = _ASSIGN( (uint8_t)0xF );
        imem.debugen_in = debugen_in;
        imem.__inst_name = __inst_name + "/imem";
        imem._assign();
