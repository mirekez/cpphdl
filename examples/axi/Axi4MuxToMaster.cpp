...
int main (int argc, char** argv)
{
    ...
    if (!noveril) {
        ...
        ok = ( ok
            && std::system((std::string("Axi4MuxToMaster_4_32_8_128/obj_dir/VAxi4MuxToMaster 4 32 8 128") + (debug?" --debug":"")).c_str()) == 0
            && std::system((std::string("Axi4MuxToMaster_8_64_16_512/obj_dir/VAxi4MuxToMaster 8 64 16 512") + (debug?" --debug":"")).c_str()) == 0
        );
    ...
    if (positional.size() >= 4) {
        ...
        if (n == 4 && addr_width == 32 && id_width == 8 && data_width == 128) {
            return !(ok && TestAxi4MuxToMaster<4,32,8,128>(debug).run());
        }
        if (n == 8 && addr_width == 64 && id_width == 16 && data_width == 512) {
            return !(ok && TestAxi4MuxToMaster<8,64,16,512>(debug).run());
        }
        std::print("Unsupported Axi4MuxToMaster test parameters: N={} ADDR={} ID={} DATA={}\n", n, addr_width, id_width, data_width);
        return 1;
    }

    ok = ok && TestAxi4MuxToMaster<4,32,8,128>(debug).run();
    ok = ok && TestAxi4MuxToMaster<8,64,16,512>(debug).run();
    return !ok;
}
